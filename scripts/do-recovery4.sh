...
if ss -ltnp 2>/dev/null | grep -q ':3000'; then
  echo 'FATAL: port still in use, aborting'
  exit 1
fi

echo '--- DELETE STALE PM2 ---'
...
echo '--- PARENT CHAIN of next-server ---'
NPID=$(ss -ltnp 2>/dev/null | grep ':3000' | grep -oP 'pid=\K[0-9]+')
echo "next-server pid=$NPID"
if [ -n "$NPID" ]; then
  PPID=$(ps -o ppid= -p $NPID 2>/dev/null | tr -d ' ')
  echo "parent_pid=$PPID"
  ps -fp $PPID 2>&1 | head -2 || true
...
curl -s -o /dev/null -w '/api/battle/leaderboard: %{http_code}\n' http://127.0.0.1:3000/api/battle/leaderboard || echo '/api/battle/leaderboard: FAILED'
echo 'BUILD_ID:'
