// scripts/tunnel-battle-18443.mjs — TCP server that forwards to bastion:3000 via SSH
...
client.on('ready', () => {
  ...
  const server = createServer((local) => {
    ...
    client.forwardOut('127.0.0.1', LOCAL_PORT, REMOTE_HOST, REMOTE_PORT, (err, stream) => {
      if (err) { console.error('[TUNNEL] forwardOut err:', err.message); local.destroy(); return; }
      local.pipe(stream);
      stream.pipe(local);
...
client.connect({ host: HOST, port: 22, username: USER, privateKey, passphrase: PASSPHRASE, readyTimeout: 30000 });
setInterval(() => {}, 1 << 30);